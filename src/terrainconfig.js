...
export class TerrainConfig extends DocumentSheet {
  ...
  static get defaultOptions() {
    return mergeObject(super.defaultOptions, {
      ...
      //title: i18n("SimpleTerrain.Configuration"),
      template: "modules/simple-terrain/templates/terrain-config.html",
      width: 400,
      height: "auto",
  ...
  getData(options) {
    let _obstacles = {};
    let _environments = canvas.terrain.getEnvironments().reduce(function (map, obj) {
      if (obj.obstacle === true) {
        _obstacles[obj.id] = i18n(obj.text);
      } else map[obj.id] = i18n(obj.text);
      return map;
    }, {});
  ...
  async _updateObject(event, formData) {
    ...
    if (formData.width !== this.object.width || formData.height !== this.object.height) {
      let reshape = this.object.object._rescaleDimensions(
        this.object,
        formData.width - this.object.width,
        formData.height - this.object.height
      );
      formData["shape.width"] = reshape.shape.width;
      formData["shape.height"] = reshape.shape.height;
      if (this.object.object.isPolygon) formData["shape.points"] = reshape.shape.points;
    }
  ...
  activateListeners(html) {
    ...
    if (setting("use-obstacles")) {
      $('select[name="environment"], select[name="obstacle"]', html).on("change", function () {
        ...
        if (
          $('select[name="environment"] option:selected', html).parent().attr("data-type") === "obstacle" &&
          $('select[name="obstacle"]', html).val() !== ""
        ) {
