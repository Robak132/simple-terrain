...
function register(module, type, ruleProvider) {
  ...
  game.settings.settings.get("simple-terrain.rule-provider").config = true;
}
...
function refreshProviderSetting() {
  ...
  for (const provider of Object.values(availableRuleProviders)) {
    ...
    let text;
    if (type === "bultin") {
      text = i18n("SimpleTerrain.rule-provider.choices.builtin");
    } else {
      let name;
      if (type === "module") {
      ...
      text = game.i18n.format(`SimpleTerrain.rule-provider.choices.${type}`, {name});
    }
    choices[provider.id] = text;
  }
  game.settings.settings.get("simple-terrain.rule-provider").choices = choices;
  game.settings.settings.get("simple-terrain.rule-provider").default = getDefaultRuleProvider();
}
...
export function updateRuleProviderVariable() {
  ...
  const configuredProvider = game.settings.get("simple-terrain", "rule-provider");
  currentRuleProvider =
    availableRuleProviders[configuredProvider] ??
    availableRuleProviders[game.settings.settings.get("simple-terrain.rule-provider")];
}
...
export function registerModule(moduleId, ruleProvider) {
  ...
  if (!module) {
    console.warn(
      `Simple Terrain | A module tried to register with the id "${moduleId}". However no active module with this id was found.` +
  ...
  // Using Simple Terrain's id is not allowed
  if (moduleId === "simple-terrain") {
    console.warn(
      `Simple Terrain | A module tried to register with the id "${moduleId}", which is not allowed. This api registration call was ignored. ` +
