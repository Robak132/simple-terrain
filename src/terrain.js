...
export class Terrain extends PlaceableObject {
  ...
  get center() {
    const {x, y, shape} = this.document;
    if (this.isPolygon) return this.centerPolygon;
    else return new PIXI.Point(x + shape.width / 2, y + shape.height / 2);
  }
  ...
  _refresh() {
    ...
    if (this.shape) this.shape.refresh();
  ...
  #refreshText() {
    ...
    const padding =
      this.document.environmentObject?.icon && (setting("show-icon") || ui.controls.activeControl === "terrain")
        ? 0
        : this.text.width / 2;
    const center = this.center;
    this.text.position.set(center.x - x - padding, center.y - y - this.text.height / 2);
  }
  ...
  #refreshIcon() {
    ...
    const padding =
      this.document.text && (setting("show-text") || ui.controls.activeControl === "terrain")
        ? this.icon.width
        : this.icon.width / 2;
    const center = this.center;
    this.icon.position.set(center.x - x - padding, center.y - y - this.icon.height / 2);
  }
  ...
  static get defaults() {
    ...
    return {
      ...
      multiple:
        sceneMult === undefined || sceneMult === ""
          ? this.layer.defaultmultiple
