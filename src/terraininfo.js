class TerrainInfo {
  ...
  get cost() {
    ...
    if (!this.reducers) return terraincost;
...
export class TemplateTerrainInfo extends TerrainInfo {
  ...
  get rawCost() {
    return this.template.data.flags["simple-terrain"].multiple;
  }
  ...
  get environment() {
    return this.template.flags["simple-terrain"]?.environment;
  }

  get obstacle() {
    return this.template.flags["simple-terrain"]?.obstacle;
  }
}
