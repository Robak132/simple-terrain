...
export class TerrainShape extends DrawingShape {
  refresh() {
    ...
    mergeObject(lStyle, {
      ...
      color: sc,
      alpha: setting("draw-border") ? drawAlpha : 0,
    ...
    if (doc.fillType) {
      ...
      if (doc.fillType === CONST.DRAWING_FILL_TYPES.PATTERN) {
        if (this.object.texture) {
          let sW =
            canvas.dimensions.size / (this.object.texture.width * (setting("terrain-image") === "diagonal" ? 2 : 1));
          let sH =
            canvas.dimensions.size / (this.object.texture.height * (setting("terrain-image") === "diagonal" ? 2 : 1));
    ...
    switch (shape.type) {
      ...
      case Drawing.SHAPE_TYPES.ELLIPSE:
        result = new PIXI.Ellipse(
          width / 2,
          height / 2,
          Math.max(Math.abs(width / 2), 0),
          Math.max(Math.abs(height / 2), 0)
        );
        break;
      case Drawing.SHAPE_TYPES.POLYGON:
  ...
  contains(x, y) {
    ...
    if (shape) return shape.contains(x, y);
    return false;
  }
  ...
  #drawRectangle() {
    ...
    if (this.document.hidden) {
      this.drawDashedPolygon(
        [0, 0, shape.width, 0, shape.width, shape.height, 0, shape.height, 0, 0],
        0,
        0,
        0,
        strokeWidth * 2,
