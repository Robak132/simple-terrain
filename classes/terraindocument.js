...
export class BaseTerrain extends foundry.abstract.Document {
  ...
  static defineSchema() {
    return {
      ...
      multiple: new foundry.data.fields.NumberField({
        required: true,
        nullable: false,
        initial: 2,
        label: "SimpleTerrain.Multiple"
      }),
      elevation: new foundry.data.fields.NumberField({
        required: true,
        nullable: false,
        initial: 0,
        label: "SimpleTerrain.Elevation"
      }),
      depth: new foundry.data.fields.NumberField({
        required: true,
        nullable: false,
        initial: 0,
        label: "SimpleTerrain.Depth"
      }),
      opacity: new foundry.data.fields.AlphaField({
        required: true,
        nullable: false,
        initial: 1,
        label: "SimpleTerrain.Opacity"
      }),
      drawcolor: new foundry.data.fields.StringField({label: "SimpleTerrain.DrawColor"}),
      environment: new foundry.data.fields.StringField({label: "SimpleTerrain.Environment"}),
      obstacle: new foundry.data.fields.StringField({label: "SimpleTerrain.Obstacle"}),
  ...
  static migrateData(data) {
     ...
     */
    if (getProperty(data, "shape.type") === undefined) setProperty(data, "shape.type", "p");
...
export class TerrainDocument extends CanvasDocumentMixin(BaseTerrain) {
  ...
  get texture() {
    ...
    if (image === "clear") return null;

    let mult = Math.clamped(this.multiple, setting("minimum-cost"), setting("maximum-cost"));
    if (mult > 4) mult = 4;
    if (mult >= 1) mult = parseInt(mult);
    if (mult < 1) mult = 0.5;

    if (mult === 1) return null;

    return `modules/simple-terrain/img/${image}${mult}x.svg`;
  }

  get environmentObject() {
    if (this.#envobj?.id === this.environment) return this.#envobj;
  ...
  static async createDocuments(data = [], context = {}) {
    ...
    for (let terrain of data) {
      ...
      if (
        (terrain.shape.type === CONST.DRAWING_TYPES.POLYGON || terrain.shape.type === CONST.DRAWING_TYPES.FREEHAND) &&
        terrain.shape.points.length < 3
      )
        continue;
      ...
      if (game.user.isGM) {
        let key = `flags.simple-terrain.terrain${document._id}`;
      ...
      if (document._object !== undefined) document.object.draw();
      else {
    ...
    if (originals.length) canvas.terrain.storeHistory("create", originals);

    if (game.user.isGM) game.socket.emit("module.simple-terrain", {action: "_createTerrain", arguments: [created]});
  ...
  static async updateDocuments(updates = [], context = {}) {
    ...
    for (let update of updates) {
      ...
      if (changes.multiple !== undefined || changes.environment !== undefined) {
        document.object.draw();
      } else document.object.refresh();

      updated.push(document);
    }

    if (originals.length && !options.isUndo) canvas.terrain.storeHistory("update", originals);

    if (game.user.isGM) game.socket.emit("module.simple-terrain", {action: "_updateTerrain", arguments: [updated]});
  ...
  static async deleteDocuments(ids = [], context = {}) {
    ...
    for (let id of deleteIds) {
      ...
      if (game.user.isGM) {
        let key = `flags.simple-terrain.-=terrain${id}`;
        updates[key] = null;

        if (!options.isUndo) originals.push(terrain);
      }
    ...
    if (!options.isUndo && originals.length) canvas.terrain.storeHistory("delete", originals);

    if (game.user.isGM) game.socket.emit("module.simple-terrain", {action: "_deleteTerrain", arguments: [ids]});

    //remove the deleted items from the scene
    if (Object.keys(updates).length) canvas.scene.update(updates);
  ...
  /*
        ...
        let embedded = data.map(d => {
            ...
            canvas.scene.setFlag('simple-terrain', 'terrain' + d._id, d);
  ...
  /*
        ...
        if (options.save === true) {
            ...
            //let updates = {};
            //updates['flags.simple-terrain.terrain' + this.document._id + '.multiple'] = data.multiple;
            let key = `flags.simple-terrain.terrain${this.document._id}`;
        ...
        if (data.environment !==undefined)
            this.updateEnvironment();
        //await canvas.scene.setFlag("simple-terrain", "terrain" + this.document._id, objectdata, {diff: false});
        //if the multiple has changed then update the image
